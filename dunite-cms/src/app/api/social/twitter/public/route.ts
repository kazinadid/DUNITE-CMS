...
import { publishTwitterPost } from '@/lib/social/twitter/publisher';
import {
  formatZodError,
  twitterPublishBodySchema,
} from '@/lib/social/twitter/validator';
...
export async function POST(req: NextRequest) {
  ...
  const parsed = twitterPublishBodySchema.safeParse(json);
  if (!parsed.success) {
  ...
  if (!canFacebookScheduleOrRetry(gate)) {
    await insertPostPublishActivityLog({
      ...
      message: 'User lacks permission for Twitter publish.',
