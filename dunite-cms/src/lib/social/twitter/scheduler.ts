...
import { publishTwitterPost, cleanupStaleTwitterPublishLocks } from './publisher';

const DEFAULT_BATCH = 10;

/** Bounded worker invoked from cron (`/api/cron/publish-scheduled`). */
export async function runScheduledTwitterPublishingTick(): Promise<{
  processed: number;
  errors: string[];
}> {
  const cleaned = await cleanupStaleTwitterPublishLocks();
  void cleaned;

  const jobs = await fetchDuePublishingJobsForWorker(DEFAULT_BATCH, 'twitter');
  ...
  for (const job of jobs) {
    ...
    await insertPostPublishActivityLog({
      ...
      message: 'Scheduled Twitter publish picked up by worker.',
      metadata: {
    ...
    try {
      ...
      await insertPostPublishActivityLog({
        ...
        message: 'Scheduled Twitter publish finished.',
        metadata: {
          organization_id: gate.organizationId,
          publishing_job_id: job.id,
        },
      ...
      await insertPostPublishActivityLog({
        ...
        message: 'Scheduled Twitter publish failed.',
        metadata: {
          organization_id: gate.organizationId,
          publishing_job_id: job.id,
